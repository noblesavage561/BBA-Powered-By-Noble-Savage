...
system_health_json="$(curl -sS http://127.0.0.1:8000/api/v1/system/health)"
...
graphql_json="$(curl -sS -X POST http://127.0.0.1:4000/ -H "Content-Type: application/json" -d '{"query":"query { health { status db_connected redis_connected } systemHealth { graphql { latencyMs requestsPerSecond errorRate historicalLatency } database { activeConnections maxConnections queryRate avgQueryTime } redis { hitRate memoryUsedMb memoryTotalMb keysCount connectedClients } agents { active pending completed } recent_logs { timestamp message type category } } fundingMatches(clientId: \"11111111-1111-1111-1111-111111111111\") { matches } }"}')"

echo "REST health: $health_json"
echo "REST system-health: $system_health_json"
...
if [[ "$health_json" == *"Internal Server Error"* || "$system_health_json" == *"Internal Server Error"* || "$doc_json" == *"Internal Server Error"* || "$funding_json" == *"Internal Server Error"* || "$categorize_json" == *"Internal Server Error"* || "$treatment_json" == *"Internal Server Error"* || "$graphql_json" == *"Internal Server Error"* ]]; then
